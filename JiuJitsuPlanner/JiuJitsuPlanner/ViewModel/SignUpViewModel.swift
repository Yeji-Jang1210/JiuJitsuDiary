...
class SignUpViewModel: ObservableObject {
            ...
            .map { (text) -> EmailStatus in
                if self.isEmailDuplicateChecked == nil {
                    if self.email.isEmpty {
                        return .EmptyEmail
                    } else if !self.validateUsableEmail() {
    ...
    func addPasswordSubscriber(){
        $password
            ...
            .map { (password, confirmPassword) -> PasswordStatus in
                ...
                else if !self.validateUsablePwd() {
            ...
            .map{ (text) -> String in
                if text.isEmpty {
                    ...
                    if !self.isBirthdayValid {
                        ...
                        if self.convertStringToDate(strDate: text) > Date.now {
                            self.isBirthdayValid = false
    ...
    func validateUsableEmail() -> Bool {
        let emailRegex = "[A-Z0-9a-z._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,64}"
        debugPrint(emailRegex)
    ...
    func validateUsablePwd() -> Bool {
        ...
        isPasswordAvailable = filter.evaluate(with: password)
        print(isPasswordAvailable.description)
        return isPasswordAvailable
    }
    
//    @objc func handleBeltInfo(_ notification: Notification){
//        guard let belt = notification.userInfo?["belt"] as? Belt else { return }
//        print("Color: \(belt.color.description) changed.")
//    }
...
//    static let beltInfoUpdated = Notification.Name("BeltInfoUpdated")
//}
